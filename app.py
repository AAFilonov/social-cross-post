...
def submit_form():
    ...
    if files:  # this block will execute if any files have been uploaded
        ...
        for i, file in enumerate(files):
            ...
            new_name = (
                new_names[i] + extension
                if new_names[i] and new_names[i] != ""
                else filename
            )
        ...
        logger.info(
            "sorted file pairs: %s", [pair[0].filename for pair in file_alt_text_pairs]
        )
        ...
        processed_files, processed_alt_texts, image_locations = process_files(
            files, alt_texts, scheduled_time
        )  # Get image_locations
        logger.debug(
            "Files after processing: %s",
            ", ".join(filename for filename, _ in processed_files),
        )
    else:
        processed_files = []
        processed_alt_texts = []
    ...
    if scheduled_time:
        ...
        with app.app_context():
            ...
            if post is None:
                ...
                logger.error(
                    "Post could not be saved to the database, skipping scheduling."
                )
                return redirect(url_for("index"))
...
def process_files(files, alt_texts, scheduled_time):
    ...
    for file, alt_text in zip(files, alt_texts):
        try:
            ...
            # if scheduled_time:
            image_url = url_for(
                "static", filename=f"temp/{folder_name}/{filename}", _external=True
            )
            # else:
